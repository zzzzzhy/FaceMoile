...
RetinaFaceNet::RetinaFaceNet(const string &model_path) {
    ...
    std::vector<std::string> param_files = {
            model_path + "/mnet_25.param",
    };
    std::vector<std::string> bin_files = {
            model_path + "/mnet_25.bin",
    };
#if NCNN_VULKAN
    ncnn::create_gpu_instance();
    retinaface.use_vulkan_compute = true;
#endif // NCNN_VULKAN
    //init
...
void
RetinaFaceNet::qsort_descent_inplace(std::vector<FaceObject> &faceobjects, int left, int right) {
    int i = left;
    int j = right;
...
void RetinaFaceNet::nms_sorted_bboxes(const std::vector<FaceObject> &faceobjects,
...
ncnn::Mat
RetinaFaceNet::generate_anchors(int base_size, const ncnn::Mat &ratios, const ncnn::Mat &scales) {
...
void RetinaFaceNet::generate_proposals(const ncnn::Mat &anchors, int feat_stride,
